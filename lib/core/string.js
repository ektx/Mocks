/*
	...
	eg: test/string/index.js
*/

import base from './base.js'

export default function string(opt) {
	let result

	if (Reflect.has(opt, 'step')) {
		return step(opt)
	}

	if (Array.isArray(opt.data)) {
		// 设置索引 
		let _index = Reflect.has(opt, 'index') 
			// 如果有指定索引，我们使用索引 
			? opt.index
			// 没有索引时，我们随机生成
			: base.integer(0, opt.data.length-1)
		
		result = opt.data[ _index ]
	} else {
		let min = opt.min || 0
		let max = opt.max || min
		let length = base.integer(min, max)

		switch (opt.data) {
			case 'zh-CN':
				result = randomZhCN( length )
				break;

			case 'CNBJX':
				length = Math.max(min, max)
				result = ChinaBaiJiaXing( length -1 )
				break;

			case 'article':
				result = englishArticle(length)
				break;

			default:
				result = randomEn( length )
		}
	}
...
function englishArticle (length) {
...
*/
function step (json) {
	return json.data+ ((json.start ? json.start : 0) + json.step)
}